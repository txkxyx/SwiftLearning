...
iOSアプリでは複数のViewが重なり合って画面のコンテンツを構成しています。ラベル、ボタン、テキスト入力などが画面を構成し、機能を提供します。
...
## まとめ
iOSアプリは、すべてのビューの親になるオブジェクトであるUIWindowを原則的に一つを持ち、その上に必要なViewを積み重ねていきます。ViewControllerにより各Viewが管理され、イベントのハンドリングや画面遷移などを行います。
